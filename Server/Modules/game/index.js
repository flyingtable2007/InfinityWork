module.exports = class {
	...
	new_user_connection(socket, username, socket_id){
			...
			try {
				data.users.forEach(async function(u){
					...
					if(check_if_this_user_is_in_world == username){
						app.database.send_to_user_socket_connection(u, {"action": "game_heartbeat", "data": data, "app": "game"});
					}
				});
		    } catch(e){}
		});
		socket.on("game_input", async function(data){
			...
			app.database.send_to_user_socket_connection(world_data.username, {"action": "game_input", data, "app": "game_world", "socket_id": world_data.socket_id});
		});
